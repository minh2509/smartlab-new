...
-- Apply once after the approved 001..012 SmartLab PostgreSQL lineage.
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_email_outbox_active_invitation
    ON email_outbox(invitation_id)
    WHERE invitation_id IS NOT NULL AND status IN ('QUEUED', 'PROCESSING');
