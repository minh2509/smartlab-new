...
public class AdminAccountServiceImpl implements AdminAccountService {
    ...
    public InvitationResponse resendInvite(String email, String adminUserId) {
        ...
        AccountInvitationEntity existingInvitation = accountInvitationRepository.findByEmail(user.getEmail())
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Invitation not found"));
        requireNoActiveDelivery(existingInvitation);
    ...
    private void requireNoActiveDelivery(AccountInvitationEntity invitation) {
        if (emailOutboxRepository.existsByInvitation_IdAndStatusIn(
                invitation.getId(), List.of(EmailOutboxStatus.QUEUED, EmailOutboxStatus.PROCESSING))) {
            throw new ResponseStatusException(HttpStatus.CONFLICT, "An invitation email is already queued for delivery");
        }
    }
