...
public class ProjectJoinRequestServiceImpl implements ProjectJoinRequestService {
    ...
    private UserEntity requireRequesterAccess(ProjectEntity project, String currentEmail) {
        if (Boolean.TRUE.equals(project.getIsPublic())) {
            return projectAccessService.requireAuthenticatedUser(currentEmail);
        }
        return projectAccessService.requireRead(project, currentEmail);
    }
