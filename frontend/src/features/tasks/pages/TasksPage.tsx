...
export function TasksPage() {
  ...
  const [evaluationMembers, setEvaluationMembers] = useState<ProjectMember[]>([])
  const [loadingEvaluationMembers, setLoadingEvaluationMembers] = useState(false)
  const [evalSelectedUserId, setEvalSelectedUserId] = useState('')
  ...
  const loadEvaluationMembers = useCallback(async () => {
    ...
    try {
      ...
      setEvaluationMembers(members)
    } catch (err: unknown) {
      setError(err instanceof Error ? err.message : 'Không thể tải thành viên dự án')
    } finally {
      ...
      {activeTab === 'evaluations' && canManageEvaluations && (
        <div className="panel">
          ...
          <form onSubmit={handleSubmitEvaluation} style={{ display: 'grid', gap: 16, maxWidth: 600 }}>
            <div>
              <label className="field-label">Thành viên cần đánh giá *</label>
              <select
                className="select"
                value={evalSelectedUserId}
                onChange={(e) => setEvalSelectedUserId(e.target.value)}
                disabled={loadingEvaluationMembers}
                required
              >
